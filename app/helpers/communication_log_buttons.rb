...
module CommunicationLogButtons
  def remote_communication_log_button(person, match: nil, needs_follow_up: false, subject: nil,
                                      body: nil, delivery_method_name: 'email',
                                      delivery_status: 'sent', button_text: 'Add Communication Log',
                                      icon_class: 'fa fa-plus', margin_class: nil, created_by: nil,
                                      path: nil, method: :post, remote: true)
    delivery_method = ContactMethod.where(name: delivery_method_name.titleize).first_or_create!(
      name: delivery_method_name.titleize, field: nil, enabled: false
    )
    link_to(create_remote_communication_logs_path(
              communication_log: {
                person_id: person&.id,
                match_id: match&.id,
  ...
  def new_communication_log_button(person, match: nil, subject: nil, body: nil, created_by: nil, needs_follow_up: true, contribution: nil)
    ...
    link_to(new_communication_log_path(contribution_id: contribution&.id, person_id: person&.id,
                                       match_id: match&.id, created_by: created_by,
                                       needs_follow_up: needs_follow_up, outbound: true,
                                       delivery_method_id: delivery_method&.id, 
                                       delivery_status: 'connected', subject: subject, body: body,
                                       created_by_id: created_by&.id, sent_at: Time.now),
            title: 'New communication log', class: 'button add-button is-primary is-outlined') do
