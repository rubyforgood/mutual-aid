...
RSpec.describe ContributionBlueprint do
  ...
  it 'returns reasonable data by default' do
    ...
    expected_data = { 'contributions' =>
      [{
        ...
        'created_at' => (contribution.created_at.to_f * 1000), # JS wants miliseconds, not seconds
        'respond_path' => nil,
        'view_path' => nil,
        'profile_path' => nil,
        'match_path' => nil,
        ...
        'contact_types' => [{
          'id' => expected_contact_method.id,
          'name' => expected_contact_method.name
        }]
      }]
    }
  ...
  it 'allows injecting a url formatter for the respond_path' do
    ...
    result = ContributionBlueprint.render(contribution,
                                          respond_path: ->(p_id) { "/testing_#{p_id}" })
  ...
  it 'allows injecting a url formatter for the profile_path' do
    ...
    result = ContributionBlueprint.render(contribution,
                                          profile_path: ->(p_id) { "/testing_#{p_id}" })
