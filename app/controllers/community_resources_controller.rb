...
class CommunityResourcesController < ApplicationController
  ...
  def index
    @community_resources = CommunityResource.includes(:organization)
  ...
  def create
    ...
    if @community_resource.save
      redirect_to @admin_status ? community_resources_path : contribution_thank_you_path,
                  notice: "Community resource was successfully submitted.#{ " We'll review." unless @admin_status }"
    else
      set_form_dropdowns
      render :new
    end
  ...
  def destroy
    ...
    redirect_to community_resources_public_path,
                notice: 'Community resource was successfully destroyed.'
  end
